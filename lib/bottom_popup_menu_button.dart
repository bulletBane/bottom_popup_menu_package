...
/// shows a popup menu, consider using [BottomPopupMenuButton].
///
/// The type `T` is the type of the value(s) the entry represents. All the
/// entries in a given menu must represent values with consistent types.
///
/// A [BottomPopupMenuEntry] may represent multiple values, for example a row with
/// several icons, or a single entry, for example a menu item with an icon (see
/// [BottomPopupMenuItem]), or no value at all (for example, [PopupMenuDivider]).
///
/// See also:
///
///  * [BottomPopupMenuItem], a popup menu entry for a single value.
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [BottomPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
abstract class BottomPopupMenuEntry<T> extends StatefulWidget {
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const BottomPopupMenuEntry({Key? key}) : super(key: key);
  ...
  ///
  /// If the [BottomPopupMenuEntry] represents a single value, this should return true
...
/// See also:
///
///  * [BottomPopupMenuItem], for the kinds of items that this widget divides.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [BottomPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class PopupMenuDivider extends BottomPopupMenuEntry<Never> {
...
///
/// Typically the [child] of a [BottomPopupMenuItem] is a [Text] widget. More
/// elaborate menus with icons can use a [ListTile]. By default, a
/// [BottomPopupMenuItem] is [kMinInteractiveDimension] pixels high. If you use a widget
...
/// {@end-tool}
///
/// See the example at [BottomPopupMenuButton] for how this example could be used in a
/// complete menu, and see the example at [CheckedPopupMenuItem] for one way to
/// keep the text of [BottomPopupMenuItem]s that use [Text] widgets in their [child]
/// slot aligned with the text of [CheckedPopupMenuItem]s or of [BottomPopupMenuItem]
/// that use a [ListTile] in their [child] slot.
///
/// See also:
///
///  * [PopupMenuDivider], which can be used to divide items from each other.
///  * [CheckedPopupMenuItem], a variant of [BottomPopupMenuItem] with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [BottomPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class BottomPopupMenuItem<T> extends BottomPopupMenuEntry<T> {
  ...
  @override
  PopupMenuItemState<T, BottomPopupMenuItem<T>> createState() =>
      PopupMenuItemState<T, BottomPopupMenuItem<T>>();
}
...
/// in the menu. By default it returns [BottomPopupMenuItem.child].
///
/// The [handleTap] method can be overridden to adjust exactly what happens when
/// the item is tapped. By default, it uses [Navigator.pop] to return the
/// [BottomPopupMenuItem.value] from the menu route.
///
/// This class takes two type arguments. The second, `W`, is the exact type of
/// the [Widget] that is using this [State]. It must be a subclass of
/// [BottomPopupMenuItem]. The first, `T`, must match the type argument of that widget
/// class, and is the type of values returned from this menu.
class PopupMenuItemState<T, W extends BottomPopupMenuItem<T>> extends State<W> {
  /// The menu item contents.
  ///
  /// Used by the [build] method.
  ///
  /// By default, this returns [BottomPopupMenuItem.child]. Override this to put
  ...
  ///
  /// By default, uses [Navigator.pop] to return the [BottomPopupMenuItem.value] from
  /// the menu route.
  @protected
  void handleTap() {
...
/// matches the default minimum height of a [BottomPopupMenuItem]. The horizontal
...
/// See also:
///
///  * [BottomPopupMenuItem], a popup menu entry for picking a command (as opposed to
///    toggling a value).
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
///  * [BottomPopupMenuButton], an [IconButton] that automatically shows a menu when
///    it is tapped.
class CheckedPopupMenuItem<T> extends BottomPopupMenuItem<T> {
...
class _PopupMenuRouteLayout extends SingleChildLayoutDelegate {
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    double y =
        size.height - childSize.height - _kMenuScreenPadding - position.bottom;
...
class _PopupMenuRoute<T> extends PopupRoute<T> {
  ...
  final RelativeRect position;
  final List<BottomPopupMenuEntry<T>> items;
...
/// [BottomPopupMenuEntry.represents] method returns true for `initialValue`, and then
/// summing the values of [BottomPopupMenuEntry.height] for all the preceding widgets
/// in the list.
///
...
///  * [PopupMenuDivider], a popup menu entry that is just a horizontal line.
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [BottomPopupMenuButton], which provides an [IconButton] that shows a menu by
...
Future<T?> showMenu<T>({
  ...
  required List<BottomPopupMenuEntry<T>> items,
  T? initialValue,
  double? elevation,
...
/// argument is the value of the [BottomPopupMenuItem] that caused its menu to be
/// dismissed.
///
/// Used by [BottomPopupMenuButton.onSelected].
typedef PopupMenuItemSelected<T> = void Function(T value);

/// Signature for the callback invoked when a [BottomPopupMenuButton] is dismissed
/// without selecting an item.
///
/// Used by [BottomPopupMenuButton.onCanceled].
typedef PopupMenuCanceled = void Function();

/// Signature used by [BottomPopupMenuButton] to lazily construct the items shown when
...
/// then [BottomPopupMenuButton] behaves like an [IconButton].
///
...
///  * [CheckedPopupMenuItem], a popup menu item with a checkmark.
///  * [showMenu], a method to dynamically show a popup menu at a given location.
class BottomPopupMenuButton<T> extends StatefulWidget {
  /// Creates a button that shows a popup menu.
  ///
  /// The [itemBuilder] argument must not be null.
  ///
  static const MethodChannel _channel =
      const MethodChannel('bottom_popup_menu_button');
  ...
  const BottomPopupMenuButton({
    ...
    this.icon,
    this.iconSize,
    this.position = const RelativeRect.fromLTRB(0, 0, 0, 0),
    this.enabled = true,
    this.shape,
    this.color,
    this.enableFeedback,
  ...
  @override
  BottomPopupMenuButtonState<T> createState() =>
...
class BottomPopupMenuButtonState<T> extends State<BottomPopupMenuButton<T>> {
  /// A method to show a popup menu with the items supplied to
  /// [BottomPopupMenuButton.itemBuilder] at the position of your [BottomPopupMenuButton].
  ///
  /// By default, it is called when the user taps the button and [BottomPopupMenuButton.enabled]
  /// is set to `true`. Moreover, you can open the button by calling the method manually.
  ///
  /// You would access your [BottomPopupMenuButtonState] using a [GlobalKey] and
  /// show the menu of the button with `globalKey.currentState.showButtonMenu`.
  void showButtonMenu() {
    final PopupMenuThemeData popupMenuTheme = PopupMenuTheme.of(context);

    final List<BottomPopupMenuEntry<T>> items = widget.itemBuilder(context);
